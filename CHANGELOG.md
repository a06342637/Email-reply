...
- 修复处理日志、系统日志、告警中心和审计日志顶部、底部分页栏中的每页数量选择器受全局表单宽度影响，在窄屏或受限卡片宽度下被压缩并把“条”挤到下一行的问题。
- 分页数量选择器改为独立的 92px 固定宽度，单位直接显示在 5/10/30/50/100 条选项中；摘要、标签和选择器均禁止换行，并保留手机端自适应布局。
- 新增分页标签结构和完整选项文案回归断言；使用生产构建在处理日志、系统日志、告警中心和审计日志的顶部、底部分页栏完成 390px 与桌面浏览器验证，未发现溢出或控制台错误。
- 当前共 156 项后端和 14 项前端测试，版本升级为 v0.20（npm 0.0.20）。
