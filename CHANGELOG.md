...
- SMTP 发件提升为左侧独立导航入口，点击后通过可复制、可刷新的深链接直接打开 SMTP 配置标签；普通系统设置与 SMTP 导航只会高亮当前对应入口。
- 自动回复任务页新增“SMTP 发件设置”快捷按钮，并明确说明收件始终由 Microsoft Graph / Gmail API 完成，发件可以独立选择邮箱 API 或 SMTP。
- 任务编辑器的 SMTP 选项明确标注“收件仍使用邮箱 API”；未创建 SMTP 配置时提供可点击的直达配置链接，创建配置后可按任务选择具体发件地址。
- 新增设置深链接和导航回归测试，重新验证 SMTP 连接、身份认证、模板渲染、内嵌图片、附件、Reply-To、发送状态与防盲重试链路；当前共 156 项后端和 14 项前端测试，版本升级为 v0.19（npm 0.0.19）。
