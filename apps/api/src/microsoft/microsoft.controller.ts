...
export class MicrosoftController {
  ...
  async callback(
    ...
    if (error || !code || !state) {
      ...
      return res.redirect(
        `${fallbackUrl.replace(/\/$/, "")}/mailboxes?oauth=error&reason=${reason}`,
      );
    }
    try {
      ...
      await this.audit
        ...
        .catch(() => undefined);
      return res.redirect(
        `${publicUrl}${result.redirectTo}?oauth=success&mailbox=${encodeURIComponent(result.mailboxId)}`,
      );
    } catch (caught) {
      await this.audit
        .write("MICROSOFT_OAUTH_FAILED", req, undefined, {
      ...
      return res.redirect(
        `${publicUrl}/mailboxes?oauth=error&reason=${reason}`,
      );
    }
  }
}
